...
const getRandomFutureDateTime = () => {
    ...
    const hours = futureDate.getHours() > 12 ? futureDate.getHours() - 12 : futureDate.getHours();
...
test.describe('Post Creation Workflow', () => {
  test.beforeEach(async ({ page }) => {
    ...
    // Step 4: Ensure successful login & redirect to dashboard
 ...
 test('User successfully creates a post and verifies it in My Posts page', async ({ page }) => {
  ...
  // Verify the first .card-content element contains the expected text
    ...
    await expect(page.locator('.card-content').first()).toContainText(randomDateTime.replace('T', ', '));
